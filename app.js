...
function openLifeProfile(name){
 ...
 $('#lifeProfileContent').innerHTML=`
   ...
   ${photos.length?`<section class="profile-section"><div class="section-head"><h3>Photos</h3><span class="muted">${photos.length}</span></div><div class="photo-grid">${photos.slice(0,18).map(p=>`<button class="photo-tile" data-lightbox-url="${escapeAttr(p.url)}" data-lightbox-caption="${escapeAttr(`${p.name} — ${fmtDate(p.date)}`)}" title="View photo from ${fmtDate(p.date)}"><img src="${escapeAttr(p.url)}" alt="${escapeAttr(p.name)} photographed ${fmtDate(p.date)}"><span>${fmtDate(p.date)}</span></button>`).join('')}</div></section>`:''}
