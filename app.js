...
function safeJsonParse(value,fallback){try{return JSON.parse(value)}catch(err){console.warn('Ignoring invalid saved data',err);return fallback}}
let pendingOps=safeJsonParse(localStorage.getItem('hs_pending_ops')||'[]',[]);
if(!Array.isArray(pendingOps))pendingOps=[];
let lastSyncAt=localStorage.getItem('hs_last_sync_at')||'';
...
function cachePending(){localStorage.setItem('hs_pending_ops',JSON.stringify(pendingOps));updateAccountStatus()}
function formatLastSync(){if(!lastSyncAt)return 'Not yet';const d=new Date(lastSyncAt);if(Number.isNaN(d.getTime()))return 'Not yet';const today=new Date();const sameDay=d.toDateString()===today.toDateString();return sameDay?d.toLocaleTimeString([], {hour:'numeric',minute:'2-digit'}):d.toLocaleDateString([], {month:'short',day:'numeric'})+' '+d.toLocaleTimeString([], {hour:'numeric',minute:'2-digit'})}
function updateAccountStatus(){const c=$('#accountConnection'),l=$('#accountLastSync'),p=$('#accountPending');if(c)c.textContent=navigator.onLine?'Online':'Offline';if(l)l.textContent=formatLastSync();if(p)p.textContent=`${pendingOps.length} change${pendingOps.length===1?'':'s'}`}
function markSynced(){lastSyncAt=new Date().toISOString();localStorage.setItem('hs_last_sync_at',lastSyncAt);updateAccountStatus()}
function setSyncStatus(state,text){const chip=$('#syncChip'),label=$('#syncText');if(chip){chip.classList.remove('syncing','synced','offline','error');chip.classList.add(state)}if(label)label.textContent=text||({syncing:'Saving…',synced:'Synced',offline:'Offline',error:'Sync issue'}[state]||'Cloud');if(state==='synced')markSynced();else updateAccountStatus()}
...
async function ensureSpecies(record){if(!currentUser||record.kind==='Seasonal/Event')return null;let q=await sb.from('species').select('id,common_name,category,scientific_name').ilike('common_name',record.name).limit(1);if(q.error)throw q.error;if(q.data?.length){const sp=q.data[0],updates={};if(sp.category!==record.group)updates.category=record.group;if(record._scientificName&&!sp.scientific_name)updates.scientific_name=record._scientificName;if(Object.keys(updates).length){const u=await sb.from('species').update(updates).eq('id',sp.id);if(u.error)throw u.error}return sp.id}const {data,error}=await sb.from('species').insert({user_id:currentUser.id,common_name:record.name,category:record.group||inferGroup(record),scientific_name:record._scientificName||null}).select('id').single();if(error)throw error;return data.id}
...
let observations=safeJsonParse(localStorage.getItem('hs_observations')||'null',null)||seed;if(!Array.isArray(observations))observations=seed;
...
let currentWeather=null,idScientificName='';
...
function renderAll(){renderToday();renderJournal();renderLife();renderSeasons();renderZones();renderSeasonalPrompts()}
...
function openModal(obs=null,presetZone=''){
 ...
 $('#modalBackdrop').classList.remove('hidden');updateObservationWeatherStrip()
}
...
$('#obsForm').addEventListener('submit',async e=>{
 ...
 const record={id:editingId||(currentUser?newUuid():Date.now()),kind,name,group:kind==='Seasonal/Event'?'Seasonal':$('#group').value,date:$('#date').value,time:$('#time').value,eventEndTime:kind==='Seasonal/Event'?$('#eventEndTime').value:'',zone:$('#zone').value,locationNote:$('#locationNote').value.trim(),detail:$('#detail').value,count:kind==='Seasonal/Event'?1:Number($('#count').value||1),measurement:kind==='Seasonal/Event'?$('#measurement').value:'',measurementUnit:kind==='Seasonal/Event'&&$('#measurement').value!==''?$('#measurementUnit').value:'',notes:$('#notes').value.trim(),isMilestone:$('#isMilestone').checked,milestoneType:$('#isMilestone').checked?$('#milestoneType').value:'',photos:photoItems.map(p=>({...p})),photo:photoItems[0]?.url||'',individualPlant:kind==='Plant'?$('#individualPlant').value.trim():'',_scientificName:idScientificName||''};
...
$('#syncChip').onclick=()=>{updateAccountStatus();$('#accountBackdrop').classList.remove('hidden')};$('#closeAccount').onclick=()=>$('#accountBackdrop').classList.add('hidden');$('#accountBackdrop').addEventListener('click',e=>{if(e.target.id==='accountBackdrop')$('#accountBackdrop').classList.add('hidden')});$('#syncNow').onclick=async()=>{await refreshFromCloud();toast('Cloud sync complete')};$('#signOut').onclick=async()=>{await ensureSupabaseClient();await sb.auth.signOut();currentUser=null;$('#accountBackdrop').classList.add('hidden');$('#app').classList.add('hidden');$('#authGate').classList.remove('hidden');setSyncStatus('offline','Cloud')};$('#importLocalData').onclick=importLocalToCloud;$('#dismissImport').onclick=()=>$('#cloudImportBanner').classList.add('hidden');
window.addEventListener('online',()=>{setSyncStatus('syncing','Reconnecting…');refreshFromCloud()});
window.addEventListener('offline',()=>setSyncStatus('offline','Offline'));
document.addEventListener('visibilitychange',()=>{if(document.visibilityState!=='visible')return;const now=Date.now();if(now-lastAutoRefreshAt<15000)return;lastAutoRefreshAt=now;refreshFromCloud()});
window.addEventListener('pageshow',e=>{if(e.persisted&&currentUser&&navigator.onLine)refreshFromCloud()});

// v1.9 — weather integration, identification assistance, and seasonal prompts
const WEATHER_KEY='hs_weather_location';
const WEATHER_CODE={0:['Clear','☀️'],1:['Mostly clear','🌤️'],2:['Partly cloudy','⛅'],3:['Cloudy','☁️'],45:['Fog','🌫️'],48:['Fog','🌫️'],51:['Light drizzle','🌦️'],53:['Drizzle','🌦️'],55:['Heavy drizzle','🌧️'],61:['Light rain','🌦️'],63:['Rain','🌧️'],65:['Heavy rain','🌧️'],71:['Light snow','🌨️'],73:['Snow','❄️'],75:['Heavy snow','❄️'],77:['Snow grains','❄️'],80:['Rain showers','🌦️'],81:['Rain showers','🌧️'],82:['Heavy showers','🌧️'],85:['Snow showers','🌨️'],86:['Heavy snow showers','❄️'],95:['Thunderstorm','⛈️'],96:['Thunderstorm / hail','⛈️'],99:['Thunderstorm / hail','⛈️']};
function weatherDesc(code){return WEATHER_CODE[Number(code)]||['Weather','🌤️']}
function savedWeatherLocation(){return safeJsonParse(localStorage.getItem(WEATHER_KEY)||'null',null)}
function setWeatherMessage(html){const card=$('#weatherCard');if(card)card.innerHTML=html}
async function fetchHomesteadWeather(force=false){
 const loc=savedWeatherLocation();
 if(!loc){setWeatherMessage('<div class="smart-empty"><strong>Set your property weather location</strong><p>Use your device location once; Homestead Seasons will remember it on this device.</p><button id="weatherSetupInline" class="secondary small">Use Current Location</button></div>');$('#weatherSetupInline')?.addEventListener('click',setupWeatherLocation);return}
 if(!navigator.onLine){if(currentWeather)renderWeather(currentWeather,loc);else setWeatherMessage('<div class="empty">Weather is unavailable while offline.</div>');return}
 const card=$('#weatherCard');if(card&&!force)card.setAttribute('aria-busy','true');
 try{
  const url=`https://api.open-meteo.com/v1/forecast?latitude=${encodeURIComponent(loc.lat)}&longitude=${encodeURIComponent(loc.lon)}&current=temperature_2m,relative_humidity_2m,apparent_temperature,weather_code,precipitation,snowfall,wind_speed_10m&daily=weather_code,temperature_2m_max,temperature_2m_min,precipitation_probability_max&temperature_unit=fahrenheit&wind_speed_unit=mph&precipitation_unit=inch&timezone=auto&forecast_days=3`;
  const r=await fetch(url,{cache:'no-store'});if(!r.ok)throw new Error('Weather service unavailable');currentWeather=await r.json();renderWeather(currentWeather,loc);updateObservationWeatherStrip();
 }catch(err){console.error(err);setWeatherMessage('<div class="empty">Could not load weather right now. Tap Refresh to try again.</div>')}
 finally{card?.removeAttribute('aria-busy')}
}
function renderWeather(data,loc){
 const c=data.current||{},d=data.daily||{},[desc,ico]=weatherDesc(c.weather_code),days=(d.time||[]).slice(0,3);
 const forecast=days.map((day,i)=>{const [dd,ii]=weatherDesc(d.weather_code?.[i]);const label=i===0?'Today':new Date(day+'T12:00:00').toLocaleDateString(undefined,{weekday:'short'});return `<div class="weather-day"><strong>${label}</strong><span>${ii}</span><small>${Math.round(d.temperature_2m_max?.[i]??0)}° / ${Math.round(d.temperature_2m_min?.[i]??0)}°</small><small>${d.precipitation_probability_max?.[i]??0}% precip.</small></div>`}).join('');
 setWeatherMessage(`<div class="weather-now"><div class="weather-icon">${ico}</div><div><div class="weather-temp">${Math.round(c.temperature_2m??0)}°F</div><div class="weather-condition">${desc}</div><div class="weather-meta">Feels ${Math.round(c.apparent_temperature??c.temperature_2m??0)}° • Humidity ${c.relative_humidity_2m??'—'}% • Wind ${Math.round(c.wind_speed_10m??0)} mph</div></div></div><div class="weather-forecast">${forecast}</div><div class="weather-location">Property weather • location saved on this device <button id="changeWeatherLocation" class="link-btn" type="button">Change</button></div>`);
 $('#changeWeatherLocation')?.addEventListener('click',setupWeatherLocation);
}
function setupWeatherLocation(){
 if(!navigator.geolocation){toast('Location is not available on this device');return}
 setWeatherMessage('<div class="empty">Requesting your property location…</div>');
 navigator.geolocation.getCurrentPosition(pos=>{const loc={lat:Number(pos.coords.latitude.toFixed(4)),lon:Number(pos.coords.longitude.toFixed(4))};localStorage.setItem(WEATHER_KEY,JSON.stringify(loc));fetchHomesteadWeather(true)},err=>{console.error(err);setWeatherMessage('<div class="smart-empty"><strong>Location was not available</strong><p>Allow location access for Homestead Seasons and try again.</p><button id="weatherRetry" class="secondary small">Try Again</button></div>');$('#weatherRetry')?.addEventListener('click',setupWeatherLocation)},{enableHighAccuracy:false,timeout:12000,maximumAge:3600000});
}
function weatherNoteText(){if(!currentWeather?.current)return'';const c=currentWeather.current,[desc]=weatherDesc(c.weather_code);return `Weather: ${Math.round(c.temperature_2m)}°F, ${desc.toLowerCase()}, humidity ${c.relative_humidity_2m}%, wind ${Math.round(c.wind_speed_10m)} mph.`}
function updateObservationWeatherStrip(){const strip=$('#obsWeatherStrip'),txt=$('#obsWeatherText');if(!strip||!txt)return;const note=weatherNoteText();strip.classList.toggle('hidden',!note);if(note)txt.textContent=note}
function renderSeasonalPrompts(){
 const box=$('#seasonalPrompts');if(!box)return;const today=new Date(),year=today.getFullYear();
 const prompts=[];
 const past=observations.filter(o=>o.date&&Number(o.date.slice(0,4))<year&&(o.isMilestone||/First|Full Bloom|Fruiting|Fall Color|Arrival|Young Seen|Frost|Freeze|Snow/i.test(`${o.detail||''} ${o.milestoneType||''}`)));
 past.forEach(o=>{const d=new Date(`${year}-${o.date.slice(5)}T12:00:00`),days=Math.round((d-today)/86400000);if(days>=-10&&days<=21){const when=days<0?'about now':days===0?'today':`in about ${days} day${days===1?'':'s'}`;prompts.push({sort:Math.abs(days),icon:o.kind==='Plant'?'🌱':o.kind==='Wildlife'?'🐦':'🍂',title:`Watch for ${o.name}`,text:`${o.milestoneType||o.detail||'Seasonal activity'} was recorded ${fmtDate(o.date)}. It may happen ${when}.`,tag:'From your history'})}});
 const month=today.getMonth();const generic=[['❄️','Winter watch','Look for tracks, overwintering birds, pond ice, and early buds.'],['🌱','Early spring watch','Watch for swelling buds, first leaves, returning birds, and early wildflowers.'],['🌸','Spring watch','First blooms, nesting behavior, young wildlife, and pollinators are worth recording.'],['🐦','Late spring watch','Watch for nesting, fledglings, full bloom, and new growth.'],['🦋','Early summer watch','Look for fireflies, butterflies, young wildlife, and fruiting plants.'],['☀️','Summer watch','Track peak insect activity, pond life, fruiting, and heat-related changes.'],['🌾','High summer watch','Watch meadow plants, seed heads, late-summer birds, and dry-weather changes.'],['🌻','Late summer watch','Look for goldenrod, asters, migrating birds, and early leaf color.'],['🍂','Early fall watch','Watch for migration, first fall color, acorns, and changing wildlife activity.'],['🍁','Fall watch','Peak color, leaf drop, migration, and first frost are useful milestones.'],['🧊','Late fall watch','Look for hard freezes, final leaf drop, pond changes, and winter arrivals.'],['❄️','Winter watch','Record snowfall, tracks, feeder activity, and pond freeze/thaw.']][month];
 prompts.sort((a,b)=>a.sort-b.sort);prompts.push({sort:99,icon:generic[0],title:generic[1],text:generic[2],tag:'Seasonal prompt'});
 box.innerHTML=prompts.slice(0,4).map(p=>`<div class="prompt-card"><div class="prompt-icon">${p.icon}</div><div><strong>${escapeHtml(p.title)}</strong><p>${escapeHtml(p.text)}</p><span class="prompt-tag">${escapeHtml(p.tag)}</span></div></div>`).join('');
}
function openIdAssistant(){if($('#kind').value==='Seasonal/Event'){toast('Identification help is for wildlife and plants');return}$('#idSearch').value=$('#name').value.trim();$('#idResults').innerHTML='';$('#idStatus').textContent='';$('#idBackdrop').classList.remove('hidden');if($('#idSearch').value)runIdSearch()}
function closeIdAssistant(){$('#idBackdrop').classList.add('hidden')}
function iconicToChoice(taxon){const iconic=taxon.iconic_taxon_name||'';if(iconic==='Plantae')return{kind:'Plant',group:inferPlantGroup(taxon.preferred_common_name||taxon.name||'')};if(iconic==='Aves')return{kind:'Wildlife',group:'Bird'};if(iconic==='Mammalia')return{kind:'Wildlife',group:'Mammal'};return{kind:'Wildlife',group:'Other Wildlife'}}
async function runIdSearch(){const q=$('#idSearch').value.trim();if(q.length<2){$('#idStatus').textContent='Type at least two characters.';return}$('#idStatus').textContent='Searching iNaturalist…';$('#idResults').innerHTML='';try{const r=await fetch(`https://api.inaturalist.org/v1/taxa/autocomplete?q=${encodeURIComponent(q)}&per_page=8&locale=en`,{cache:'no-store'});if(!r.ok)throw new Error('Identification service unavailable');const data=await r.json(),results=data.results||[];$('#idStatus').textContent=results.length?`${results.length} possible match${results.length===1?'':'es'} — confirm before using.`:'No matches found.';$('#idResults').innerHTML=results.map((t,i)=>{const common=t.preferred_common_name||t.english_common_name||t.name,choice=iconicToChoice(t),ico=choice.kind==='Plant'?'🌿':choice.group==='Bird'?'🐦':choice.group==='Mammal'?'🦌':'🦋';return `<div class="id-result"><span class="id-icon">${ico}</span><div class="id-body"><strong>${escapeHtml(common)}</strong><em>${escapeHtml(t.name||'')}</em><small>${escapeHtml(t.rank||'taxon')} • ${escapeHtml(t.iconic_taxon_name||'')}</small></div><button class="secondary small use-id" data-id-use="${i}" type="button">Use</button></div>`}).join('');window._hsIdResults=results}catch(err){console.error(err);$('#idStatus').textContent='Could not reach the identification service. Try again when online.'}}
function useIdResult(index){const t=window._hsIdResults?.[Number(index)];if(!t)return;const common=t.preferred_common_name||t.english_common_name||t.name,choice=iconicToChoice(t);$('#kind').value=choice.kind;$$('.seg').forEach(x=>x.classList.toggle('active',x.dataset.kind===choice.kind));updateDetailOptions();updateKindFields();$('#name').value=common;$('#group').value=choice.group;idScientificName=t.name||'';closeIdAssistant();toast(`Using ${common}`)}
$('#weatherRefresh')?.addEventListener('click',()=>fetchHomesteadWeather(true));$('#weatherSetup')?.addEventListener('click',setupWeatherLocation);
$('#identifyBtn')?.addEventListener('click',openIdAssistant);$('#closeIdAssistant')?.addEventListener('click',closeIdAssistant);$('#idBackdrop')?.addEventListener('click',e=>{if(e.target.id==='idBackdrop')closeIdAssistant()});$('#runIdSearch')?.addEventListener('click',runIdSearch);$('#idSearch')?.addEventListener('keydown',e=>{if(e.key==='Enter'){e.preventDefault();runIdSearch()}});$('#idResults')?.addEventListener('click',e=>{const b=e.target.closest('[data-id-use]');if(b)useIdResult(b.dataset.idUse)});
$('#addWeatherToNotes')?.addEventListener('click',()=>{const note=weatherNoteText();if(!note)return;const n=$('#notes');n.value=(n.value.trim()?n.value.trim()+'\n':'')+note;toast('Weather added to notes')});
fetchHomesteadWeather();

updateAccountStatus();
initAuth();
ensureSupabaseClient().then(client=>{client.auth.onAuthStateChange((event,session)=>{if(event==='SIGNED_OUT'){currentUser=null;cloudReady=false;$('#app').classList.add('hidden');$('#authGate').classList.remove('hidden');setSyncStatus('offline','Cloud')}else if(event==='TOKEN_REFRESHED'){updateAccountStatus()}})}).catch(()=>{});
